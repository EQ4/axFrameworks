...
axWaveform::Info::Info(const ax::StringPairVector& attributes)
{
    SetAttributes(attributes);
}

ax::StringVector axWaveform::Info::GetParamNameList() const
{
    return ax::StringVector{"bg_color",
                          "waveform_color",
                          "contour_color",
...
void axWaveform::Info::SetAttribute(const ax::StringPair& attribute)
{
...
axWidget* axWaveform::Builder::Create(const ax::StringPairVector& attributes)
{
...
void axWaveform::SetAudioBuffer(ax::Audio::Buffer* buffer)
{
    _audioBuffer = buffer;
    _zoom = 1.0;
    _leftPos = 0.0;
...
void axWaveform::SetGain(const double& gain)
{
    double v = ax::Utils::Clamp<double>(gain, 0.0, 1.0);
    ax::FloatRange range(0.0, 2.0);
    
    _gain = range.GetValueFromZeroToOne(v);
    Update();
}
...
ax::FloatRange axWaveform::GetBorders() const
{
    if(_audioBuffer != nullptr)
    {
        axBufferInfo b_info = _audioBuffer->GetBufferInfo();
        ...
        return ax::FloatRange(left_pos * r, right_pos * r);
    }
    return ax::FloatRange(0.0, 1.0);
}
    ...
    if(_audioBuffer != nullptr)
    {
        ...
        for(int i = 1; i < floor(nSamplesToProcess); i++, index++)
        {
            ...
            if(int(x_pos_left) != int(x_pos_right))
            {
                ...
                {
                    // Pixel value.
                    int y_pixel_left = middle_y - min_value_pixel * 0.9 * middle_y * _gain;
                    int y_pixel_right = middle_y - max_value_pixel * 0.9 * middle_y * _gain;
                    ...
                    y_pixel_left = middle_y - l_value * 0.9 * middle_y * _gain;
                    y_pixel_right = middle_y - r_value * 0.9 * middle_y * _gain;
                ...
                {
                    // Pixel value.
                    int y_pixel_left = middle_y - l_value * 0.9 * middle_y * _gain;
                    int y_pixel_right = middle_y - r_value * 0.9 * middle_y  * _gain;
