...
(const FilterParametric::FilterInfo<double>& info)
{
    ...
    double w0 = ax::Audio::TwoPi * info.freq / sr;
    double c = cosf(w0);
...
FilterParametric::FilterParametric(axWindow* parent, const ax::Rect& rect):
axPanel(parent, rect),
_font(0)
{
    _font.SetFontSize(9);
    ...
    for(int i = 0; i < 300; i++)
    {
        _freqs[i] = ax::Audio::LogarithmicInterpole<double>
                ...
                const int& sr)
{
    ...
    if(filter.info.active)
    {
        ...
        {
            const double w = 2.0 * ax::Audio::Pi * freqs[i] / double(sr);
...
//    using axAudio::Pow2;
    ...
    for(int i = 0; i < _linesFreq.size(); i++)
    {
        ...
        gc.DrawLine(ax::Point(x_pos, 1.0), ax::Point(x_pos, rect.size.y));
    }
    ...
    for(int i = 0; i < 15; i++)
    {
        ...
        gc.DrawLine(ax::Point(1.0, y_pos), ax::Point(rect.size.x, y_pos));
    }
    
    // Draw frequency response.
    gc.SetSmoothLine(2.0);
    gc.SetColor(ax::Color(0.8, 0.4, 0.0));
    ...
    for(int i = 1; i < _freqs.size(); i++)
    {
        ...
        const ax::Point lpt(x_left, rect.size.y * (1.0 - points[i - 1] * 0.5));
        const ax::Point rpt(x_right, rect.size.y * (1.0 - points[i] * 0.5));
    ...
    for(int i = 0; i < 8; i++)
    {
        ...
        {
            gc.SetColor(ax::Color(0.9, 0.4, 0.0));
            const double x = (log(f[i]) - log15) * mult_const;
            
            ax::Point pt_pos(x, rect.size.y * (1.0 - pt[i] * 0.5));
            gc.DrawPoint(pt_pos, 11);
            
            gc.SetColor(ax::Color(1.0));
            
            gc.DrawString(_font, std::to_string(i + 1),
                          pt_pos - ax::Point(3, 6));
        }
    }
